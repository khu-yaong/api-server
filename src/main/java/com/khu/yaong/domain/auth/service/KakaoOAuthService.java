...
public class KakaoOAuthService {
    ...
    public MemberLoginResponseDto loginKakao(String authorizationCode) {
        ...
        String token = jwtTokenProvider.createToken(savedMember.get().getId(),savedMember.get().getRole());
        return new MemberLoginResponseDto(token, savedMember.get().getId(), savedMember.get().getUsername(), savedMember.get().getRole(), savedMember.get().getProfileImage());
    }
